<%@ Page Title="" Language="C#" MasterPageFile="~/OutletMaster.master" AutoEventWireup="true" CodeFile="AssetConfirm.aspx.cs" Inherits="Asset_AssetConfirm" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="Server">
    ...
    <script>
        ...
        function ViewAssetHistory(assetID) {
            $.ajax({
                url: 'AssetConfirm.aspx/GetAssetTransferDetailsByIDForPopup',
                type: 'POST',
                dataType: 'json',
                //data:{},
        ...
        function ViewConfirmDetails(assetID) {
            $.ajax({
                url: 'AssetConfirm.aspx/GetAssetConfirmDetailsByIDForPopup',
                type: 'POST',
                dataType: 'json',
                data: JSON.stringify({ assetID: assetID }),
                contentType: 'application/json',
        ...
        function ViewAttachGrid(assetTransferID, type) {
            var comp = false;
            $.ajax({
                url: 'AssetConfirm.aspx/GetAssetTransferAttachmentsForPopup',
                type: 'POST',
                dataType: 'json',
        ...
        function CheckForExtraAssets() {
            var astCode = $('#body_txtAssetCodeAdd').val();
            var astName = $('#body_txtAssetNameAdd').val();
            var astRemark = $('#body_txtExtraRemarksAdd').val();

            var ddl = document.getElementById("<%=ddlAssetBrand.ClientID%>");
            var astBrand = ddl.options[ddl.selectedIndex].value;
            var ddl = document.getElementById("<%=ddlAssetSize.ClientID%>");
            var astSize = ddl.options[ddl.selectedIndex].value;

            if (astCode == "" && astName == "" && astRemark != "") {
                alert(" Please enter AssetCode & Name in Extra Asset Row");
                $('#body_txtAssetCodeAdd').focus();
                event.preventDefault();
                return false;
            }
            if ((astCode == "" && astName != "") || (astCode != "" && astName == "")) {
                alert(" Please enter AssetCode & Name in Extra Asset Row");
                event.preventDefault();
                return false;
            }
            if ((astCode != "" && astName != "" && astRemark == "")) {
                alert(" Please enter Remarks in Extra Asset Row");
                $('#body_txtExtraRemarksAdd').focus();
                event.preventDefault();
                return false;
            }

            if ((astCode != "" && astName != "" && astRemark != "") && (astBrand == 0 || astSize == 0)) {
                alert(" Please Select AssetBrand & AssetSize in Extra Asset Row");
                event.preventDefault();
                return false;
            }

            //var astCodeLess = $('#body_txtAssetCodeLess').val();
            //var astNameLess = $('#body_txtAssetNameLess').val();
            //var astRemarkLess = $('#body_txtExtraRemarksLess').val();

            //if (astCodeLess == "" && astNameLess == "" && astRemarkLess != "") {
            //    alert(" Please enter AssetCode & Name in Extra Asset 'Less' Row");
            //    $('#body_txtAssetCodeLess').focus();
            //    event.preventDefault();
            //    return false;
            //}
            //if ((astCodeLess == "" && astNameLess != "") || (astCodeLess != "" && astNameLess == "")) {
            //    alert(" Please enter AssetCode & Name in Extra Asset 'Less' Row");
            //    event.preventDefault();
            //    return false;
            //}
            //if ((astCodeLess != "" && astNameLess != "" && astRemarkLess == "")) {
            //    alert(" Please enter Remarks in Extra Asset 'Less' Row");
            //    $('#body_txtExtraRemarksLess').focus();
            //    event.preventDefault();
            //    return false;
            //}
        }
        ...
        function CheckAllAcceptRecords(Checkbox) {
            ...
            var headerchk = document.getElementById(GridVwHeaderChckbox.id + '_chkboxRejectAll');
            headerchk.checked = false;
        }
        function CheckAllRejectRecords(Checkbox) {
            var GridVwHeaderChckbox = document.getElementById("<%=gvAsset.ClientID %>");
            for (i = 1; i < GridVwHeaderChckbox.rows.length; i++) {
                GridVwHeaderChckbox.rows[i].cells[3].getElementsByTagName("INPUT")[0].checked = Checkbox.checked;
                GridVwHeaderChckbox.rows[i].cells[2].getElementsByTagName("INPUT")[0].checked = false;
            }

            var headerchk = document.getElementById(GridVwHeaderChckbox.id + '_chkboxAcceptAll');
            headerchk.checked = false;
        }

        function CheckAccept_Click(chkbox) {

            var chkRej = document.getElementById(chkbox.id.replace("chkAccept", "chkReject"));

            var cnt = 1;
            var gridView = document.getElementById("<%=gvAsset.ClientID %>");
            var headerchk = document.getElementById(gridView.id + '_chkboxAcceptAll');

            if (chkbox.checked == false) {
                headerchk.checked = false;
                chkRej.checked = true;
                var headchk = document.getElementById(gridView.id + '_chkboxRejectAll');
                ...
                if (gridView.rows.length == cnt) {
                    ...
                    headchk.checked = false;
                    for (i = 1; i < gridView.rows.length; i++) {
                        gridView.rows[i].cells[3].getElementsByTagName("INPUT")[0].checked = false;
                    }
                }
            }
        }

        function CheckReject_Click(chkbox) {
            var chkAcc = document.getElementById(chkbox.id.replace("chkReject", "chkAccept"));

            var cnt = 1;
            var gridView = document.getElementById("<%=gvAsset.ClientID %>");
            var headerchk = document.getElementById(gridView.id + '_chkboxRejectAll');

            if (chkbox.checked == false) {
                headerchk.checked = false;
                chkAcc.checked = true;
                var headchk = document.getElementById(gridView.id + '_chkboxAcceptAll');
                headchk.checked = false;
            }
            else {
                chkAcc.checked = false;
                for (i = 1; i < gridView.rows.length; i++) {
                    if (gridView.rows[i].cells[3].getElementsByTagName("INPUT")[0].checked) {
                        cnt++;
                    }
                }
                var headchk = document.getElementById(gridView.id + '_chkboxAcceptAll');
                ...
                if (gridView.rows.length == cnt) {
                    ...
                    headchk.checked = false;
                    for (i = 1; i < gridView.rows.length; i++) {
                        gridView.rows[i].cells[2].getElementsByTagName("INPUT")[0].checked = false;
                    }
                }
            }
        }
    ...
    <div class="panel panel-default">
        <div class="panel-body _masterForm">
            ...
            <div style="overflow-x: auto; overflow-y: auto" class="tdiv">
                ...
                <asp:GridView runat="server" ID="gvAsset" CssClass="gvAsset table" AutoGenerateColumns="false" HeaderStyle-CssClass="table-header-gradient" OnRowDataBound="gvAsset_RowDataBound" EmptyDataText="No Item Found." OnPreRender="gvAsset_PreRender">
                    <Columns>
                        ...
                        <asp:TemplateField HeaderText="Accept" ItemStyle-HorizontalAlign="Left" HeaderStyle-HorizontalAlign="Center">
                            <HeaderTemplate>
                                Accept
                                <asp:CheckBox ID="chkboxAcceptAll" runat="server" CssClass="form-control" onclick="CheckAllAcceptRecords(this);" />
                            </HeaderTemplate>
                            <ItemTemplate>
                                <asp:CheckBox ID="chkAccept" runat="server" CssClass="form-control chkAccept" onclick="CheckAccept_Click(this)" />
                            </ItemTemplate>
                        </asp:TemplateField>
                        <asp:TemplateField HeaderText="Reject" ItemStyle-HorizontalAlign="Left" HeaderStyle-HorizontalAlign="Center">
                            <HeaderTemplate>
                                Reject
                                <asp:CheckBox ID="chkboxRejectAll" runat="server" CssClass="form-control" onclick="CheckAllRejectRecords(this);" />
                            </HeaderTemplate>
                            <ItemTemplate>
                                <asp:CheckBox ID="chkReject" runat="server" CssClass="form-control chkReject" onclick="CheckReject_Click(this)" />
                            </ItemTemplate>
                        </asp:TemplateField>
                        ...
                        <asp:TemplateField HeaderText="History">
                            <ItemTemplate>
                                <a onclick="ViewAssetHistory('<%#Eval("AssetID")%>');">
                                    <img id='imgAstCF' src='../Images/search.png' style='cursor: pointer;' /></a>
                            </ItemTemplate>
                        </asp:TemplateField>
            ...
            <br />
            <b>Other/Extra Assets Details</b>
            <table id="tblExtra" class="table" border="1">
                ...
                <tr>
                    <td>Extra</td>
                    <td>
                        <asp:TextBox ID="txtAssetCodeAdd" runat="server" CssClass="form-control"></asp:TextBox></td>
                    <td>
                        <asp:TextBox ID="txtAssetNameAdd" runat="server" CssClass="form-control"></asp:TextBox></td>
                    <td>
                        <asp:DropDownList ID="ddlAssetBrand" runat="server" DataSourceID="edsddlAssetBrand" DataTextField="AssetBrandName"
                            DataValueField="AssetBrandID" AppendDataBoundItems="true" CssClass="form-control">
                    ...
                    <td>
                        <asp:DropDownList ID="ddlAssetSize" runat="server" DataSourceID="edsddlAssetSize" DataTextField="AssetSizeName"
                            DataValueField="AssetSizeID" AppendDataBoundItems="true" CssClass="form-control">
                            <asp:ListItem Value="0" Text="---Select---"></asp:ListItem>
                        </asp:DropDownList>
                    ...
                    <td>
                        <asp:TextBox ID="txtExtraRemarksAdd" runat="server" CssClass="form-control"></asp:TextBox></td>
                </tr>
                <%-- <tr>
                    <td>Less</td>
                    <td><asp:TextBox ID="txtAssetCodeLess" runat="server" CssClass="form-control"></asp:TextBox></td>
                    <td><asp:TextBox ID="txtAssetNameLess" runat="server" CssClass="form-control"></asp:TextBox></td>
                     <td><asp:TextBox ID="txtExtraRemarksLess" runat="server" CssClass="form-control"></asp:TextBox></td>
                </tr>--%>
            </table>
