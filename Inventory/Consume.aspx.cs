...
public partial class Inventory_Consume : System.Web.UI.Page
{
    #region Property
    ...
    public void ClearAllInputs()
    {
        using (DDMSEntities ctx = new DDMSEntities())
        {
            if (ctx.OSEQs.Any(x => x.ParentID == ParentID && !x.IsDeleted && x.Type == "C" && EntityFunctions.TruncateTime(x.FromDate) <= EntityFunctions.TruncateTime(DateTime.Now) && EntityFunctions.TruncateTime(x.ToDate) >= EntityFunctions.TruncateTime(DateTime.Now)))
            {
            }
            else
            {
                ScriptManager.RegisterStartupScript(this, this.GetType(), "ModelMsg", "ModelMsg('Invoice Series Not Found for Consume. !',3);", true);
            ...
            ORSNs = ctx.ORSNs.Where(x => x.Active && x.Type == "R").Select(x => new DisData { Text = x.ReasonName, Value = x.ReasonID }).ToList();
        }
    }
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        try
        {
            using (DDMSEntities ctx = new DDMSEntities())
            {
                ...
                OSEQ objOSEQ = ctx.OSEQs.Where(x => x.ParentID == ParentID && !x.IsDeleted && x.Type == "C" && EntityFunctions.TruncateTime(x.FromDate) <= EntityFunctions.TruncateTime(date) && EntityFunctions.TruncateTime(x.ToDate) >= EntityFunctions.TruncateTime(date)).FirstOrDefault();
