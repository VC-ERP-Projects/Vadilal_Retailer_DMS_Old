...
public partial class Master_CategoryMaster : System.Web.UI.Page
{
    #region Declaration
    ...
    private void ClearAllInput()
    {
        ...
        try
        {
            ...
            Cm.CommandText = "GetCategoryMasterData";
    ...
    protected void saveData_Click(object sender, EventArgs e)
    {
        try
        {
            using (DDMSEntities ctx = new DDMSEntities())
            {
                ...
                OCTG objCtg = ctx.OCTGs.FirstOrDefault(x => x.CategoryId == CategoryId);
                if (objCtg != null)
                {
                    objCtg.CategoryCode = txtCategoryCode.Text;
                    objCtg.Description = txtDescription.Text;
                    objCtg.Active = chkActive.Checked;
                    objCtg.UpdateBy = UserID;
                    objCtg.UpdatedDate = DateTime.Now;
                }
                else
                {
                    OCTG objectCTG = new OCTG();
                    objectCTG.CategoryCode = txtCategoryCode.Text;
                    objectCTG.Description = txtDescription.Text;
                    objectCTG.Active = chkActive.Checked;
                    objectCTG.Deleted = false;
                    objectCTG.CreatedBy = UserID;
                    objectCTG.CreatedDate = DateTime.Now;
                    objectCTG.UpdateBy = UserID;
                    objectCTG.UpdatedDate = DateTime.Now;
                    ctx.OCTGs.Add(objectCTG);
                }
                ctx.SaveChanges();
        ...
        try
        {
            if (!string.IsNullOrEmpty(e.CommandArgument.ToString()))
            {
                ...
                using (DDMSEntities ctx = new DDMSEntities())
                {
                    ...
                    {
                        OCTG objCtg = ctx.OCTGs.FirstOrDefault(x => x.CategoryId == CategoryId && x.Deleted == false);
                        hdnCategoryId.Value = objCtg.CategoryId.ToString();
                        txtCategoryCode.Text = objCtg.CategoryCode;
                    ...
                    {
                        OCTG objOSEQ = ctx.OCTGs.FirstOrDefault(x => x.CategoryId == CategoryId && x.Deleted == false);
                        objOSEQ.Deleted = true;
                        objOSEQ.UpdatedDate = DateTime.Now;
                        objOSEQ.UpdateBy = UserID;
                        ctx.SaveChanges();
                        ClearAllInput();
                    }
                }
