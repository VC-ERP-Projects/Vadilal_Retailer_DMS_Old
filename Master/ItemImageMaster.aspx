<%@ Page Title="" Language="C#" MasterPageFile="~/MainMaster.master" AutoEventWireup="true" CodeFile="ItemImageMaster.aspx.cs" Inherits="Master_DistRegionCompanyMapping" %>
    ...
    <script type="text/javascript">
            ...
            $(document).on('keyup', '.AutoRegion', function () {
                ...
                $('#AutoRegion' + col1).autocomplete({
                    source: function (request, response) {
                        $.ajax({
                            type: "POST",
                            url: 'ItemImageMaster.aspx/SearchRegion',
                            dataType: "json",
                    ...
                    select: function (event, ui) {
                        ...
                        $('#hdnRegionId' + col1).val(ui.item.id);
                        $('#AutoDist' + col1).val("");
        ...
        function CheckDuplicateData(pRegioCode, row, ChkType) {
            ...
            $('#tblEmpClaimLevel  > tbody > tr').each(function (row1, tr) {
                ...
                var RegionCode = $("input[name='AutoRegion']", this).val() != "" ? $("input[name='AutoRegion']", this).val().split("-")[0].trim() : "";
                ...
                if (RgnId != "") {
                    if (parseInt(row) != parseInt(LineNum)) {
                        if (Item == RegionCode) {
                            ...
                            errormsg = 'Item is already set for = ' + pRegioCode + ' at row : ' + rowCnt_Claim;
        ...
        function FillData() {
            ...
            var IsValid = true;
            $.ajax({
                url: 'ItemImageMaster.aspx/LoadData',
                type: 'POST',
                dataType: 'json',
                contentType: 'application/json; charset=utf-8',
                data: JSON.stringify({ optionId: 0, Division: DivisionId }),
                async: false,
                        ...
                        if (items.length > 0) {
                            ...
                            for (var i = 0; i < items.length; i++) {
                                ...
                                if (items[i].Logo == "") {
                                    ...
                                    PreApprovalAttachent = "<a href='/Images/ItemIamges/" + items[i].Logo + "'target='_blank' class='form-control LogoImage'>" + items[i].Logo + "</a>";
                                }
                                if (items[i].Logo == "") {
                                    ImageName = "No Image";
                                }
                                ...
                                strEmp = "<tr id='trUserMap" + indE + "'>"
                                    ...
                                    + "<td class='tdRegion'><input type='text' id='AutoRegion" + indE + "' name='AutoRegion' onchange='ChangeData(this);' class='form-control search AutoRegion ' value='" + items[i].ItemName + "' disabled='false' /></td>"
                                    ...
                                    /*+ "<td class='tdDist'><img id='img" + indE + "' imageno='" + indE + "' name='img' src='" + items[i].logo + "' alt='' class='form-control LogoImage' data-zoom-image='" + items[i].ItemName + "'  /></td>"*/
                                    + "<td>" + ImageName + "</td>"
                                    ...
                                    + "<input type='hidden' class='hdnRegionId' id='hdnRegionId" + indE + "' name='hdnRegionId' value='" + items[i].ItemID + "'/></td>"
                                    ...
                                    + "<input type='hidden' class='hdnCompanyMappingId' id='hdnCompanyMappingId" + indE + "' name='hdnCompanyMappingId' value='" + items[i].ItemMappingID + "' /></td>"
        ...
        function btnSubmit_Click() {
            ...
            $('#tblEmpClaimLevel  > tbody > tr').each(function (row, tr) {
                ...
                if ((Region != "" && LogoFile != "") && (IsChange == "1")) {
                    ...
                    /*var Region = $("input[name='hdnRegionId']", this).val().split('-')[0];*/
                    var Region = $("input[name='hdnRegionId']", this).val();
                    var IsChange = $("input[name='IsChange']", this).val().trim();
                    var ImgName = $("input[name='hdnFileName1']", this).val().trim();
            ...
            if (IsValid) {
                var sv = $.ajax({
                    url: 'ItemImageMaster.aspx/SaveData',
                    type: 'POST',
                    //async: false,
                    dataType: 'json',
                    // traditional: true,
                    data: JSON.stringify({ hidJsonInputUnitMapping: UserUnitMappingData, OptionId: 0 }),
                    contentType: 'application/json; charset=utf-8'
                });
        ...
        function GetReport() {
            ...
            if ($('.chkIsReport').find('input').is(':checked')) {
                ...
                $('.gvEmpClaimLevelHistory tbody').empty();
                $.ajax({
                    url: 'ItemImageMaster.aspx/LoadReport',
                    type: 'POST',
                    dataType: 'json',
                    async: false,
                    contentType: 'application/json; charset=utf-8',
                        ...
                        <table id="tblEmpClaimLevel" class="table table-bordered nowrap" border="1" tabindex="8" style="border-collapse: collapse; font-size: 10px; width: 100%">
                            <thead>
                                <tr class="table-header-gradient">
                                    ...
                                    <th style="width: 60%; padding-left: 10px !important;">Item</th>
