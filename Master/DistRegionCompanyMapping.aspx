<%@ Page Title="" Language="C#" MasterPageFile="~/MainMaster.master" AutoEventWireup="true" CodeFile="DistRegionCompanyMapping.aspx.cs" Inherits="Master_DistRegionCompanyMapping" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="Server">
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            $(document).on('keyup', '.AutoRegion', function () {
                ...
                $('#AutoRegion' + col1).autocomplete({
                    source: function (request, response) {
                        $.ajax({
                            type: "POST",
                            url: 'DistRegionCompanyMapping.aspx/SearchRegion',
                            dataType: "json",
            ...
            $('.AutoRegion').on('blur', function (e, ui) {
                ...
                if ($('#AutoRegion' + col1).val() != "") {
                    ...
                    CheckDuplicateData($('#AutoRegion' + col1).val().trim(), $('#AutoCompany' + col1).val().trim(), col1, 1);
                }
            });
            ...
            $(document).on('keyup', '.AutoCompany', function () {
                var textValue = $(this).val();
                //var currentRow = $(this).closest("tr");
                //var col1 = currentRow.find("td:eq(0)").text();
                var txtId = $(this).attr("id");
                var col1 = txtId.replace("AutoCompany", '');
                $('#AutoCompany' + col1).autocomplete({
                    source: function (request, response) {
                        //var RegionId = $("#AutoRegion" + col1).val() != "" && $("#AutoRegion" + col1).val() != undefined ? $("#AutoRegion" + col1).val().split("-")[2].trim() : "0";
                        var RegionId = 0;
                        $.ajax({
                            type: "POST",
                            url: 'DistRegionCompanyMapping.aspx/SearchSalesOrg',
                            dataType: "json",
                    ...
                    select: function (event, ui) {
                        $('#AutoCompany' + col1).val(ui.item.value + " ");
                        $('#hdnCompanyId' + col1).val(ui.item.value.split("#")[2].trim());
                    },
                    change: function (event, ui) {
                        if (!ui.item) {
            ...
            $('.AutoCompany').on('autocompleteselect', function (e, ui) {
                var currentRow = $(this).closest("tr");
                var col1 = currentRow.find("td:eq(0)").text();
                $('#AutoCompany' + col1).val(ui.item.value);
            ...
            $('.AutoCompany').on('change keyup', function () {
                var txtId = $(this).attr("id");
                var col1 = txtId.replace("AutoCompany", '');
                if ($('#AutoCompany' + col1).val() == "") {
                    ClearClaimRow(col1);
                }
            });

            $('.AutoCompany').on('blur', function (e, ui) {
                var txtId = $(this).attr("id");
                var col1 = txtId.replace("AutoCompany", '');
                if ($('#AutoCompany' + col1).val() != "") {
                    if ($('#AutoCompany' + col1).val().indexOf('#') == -1) {
                        ModelMsg("Select Proper Sales Organization", 3);
                        $('#AutoCompany' + col1).val("");
                        $('#hdnCompanyId' + col1).val('0');
                        return;
                    }
                    var txt = $('#AutoCompany' + col1).val().trim();
                    if (txt == "undefined" || txt == "") {
                        //ModelMsg("Enter Item Code Or Name", 3);
                        return false;
                    }
                    ...
                    CheckDuplicateData($('#AutoRegion' + col1).val().trim(), $('#AutoCompany' + col1).val().trim(), col1, 2);
            ...
            $(document).on('click', '.btnEdit', function () {
                ...
                if (checkBoxes.prop("checked") == true) {
                    ...
                    $(this).closest('tr').find('.search,.checkbox,.CompanyLogo').prop('disabled', false);
                    $(this).val('Update');
                } else {
                    checkBoxes.prop("checked", true);
                    $(this).closest('tr').find('.search,.checkbox,.CompanyLogo').prop('disabled', true);
                    $(this).val('Edit');
                }
            });
        ...
        function CheckDuplicateData(pRegioCode, pComoanyCode, row, ChkType) {
            ...
            $('#tblEmpClaimLevel  > tbody > tr').each(function (row1, tr) {
                ...
                var CompanyCode = $("input[name='AutoCompany']", this).val() != "" ? $("input[name='AutoCompany']", this).val().split("#")[2].trim() : "";
        ...
        function FillData() {
            ...
            var IsValid = true;
            $.ajax({
                url: 'DistRegionCompanyMapping.aspx/LoadData',
                type: 'POST',
                dataType: 'json',
                contentType: 'application/json; charset=utf-8',
                        ...
                        if (items.length > 0) {
                            ...
                            for (var i = 0; i < items.length; i++) {
                                ...
                                strEmp = "<tr id='trUserMap" + indE + "'>"
                                    ...
                                    + "<input type='hidden' class='hdnCompanyMappingId' id='hdnCompanyMappingId" + indE + "' name='hdnCompanyMappingId' value='" + items[i].CompanyMappingId + "' /></td>"
        ...
        function AddMoreRow() {
            ...
            strEmp = "<tr id='trUserMap" + indE + "'>"
                ...
                + "<td class='tdDist'><input type='file' id='CompanyLogo" + indE + "' name='CompanyLogo' onchange='PreviewImageForLabel(this)' class='form-control CompanyLogo' /></td>"
            ...
            $('#AutoRegion' + indE).on('blur', function (e, ui) {
                ...
                if ($('#AutoRegion' + indE).val().trim() != "") {
                    ...
                    CheckDuplicateData($('#AutoRegion' + indE).val().trim(), $('#AutoCompany' + indE).val().trim(), indE, 1);
                }
            });
            ...
            $('#AutoCompany' + indE).autocomplete({
                ...
                position: { collision: "flip" },
                select: function (event, ui) {
                    $('#hdnCompanyId' + indE).val(ui.item.value.split('#')[2].trim());
                    $('#AutoCompany' + indE).val(ui.item.value + " ");
                },
                change: function (event, ui) {
                    if (!ui.item) {
                    }
            ...
            $('#AutoCompany' + indE).on('autocompleteselect', function (e, ui) {
                $('#AutoCompany' + indE).val(ui.item.value);
            });

            $('#AutoCompany' + indE).on('change keyup', function () {
                if ($('#AutoCompany' + indE).val() == "") {
                    ClearClaimRow(indE);
                }
            });

            $('#AutoCompany' + indE).on('blur', function (e, ui) {
                if ($('#AutoCompany' + indE).val().trim() != "") {
                    if ($('#AutoCompany' + indE).val().indexOf('#') == -1) {
                        ModelMsg("Select Proper Sales Organization", 3);
                        $('#AutoCompany' + indE).val("");
                        $('#hdnCompanyId' + indE).val('0');
                        return;
                    }
                    var txt = $('#AutoCompany' + indE).val().trim();
                    if (txt == "undefined" || txt == "") {
                        return false;
                    }
                    var lineNum = 1;
                    ...
                    CheckDuplicateData($('#AutoRegion' + indE).val().trim(), $('#AutoCompany' + indE).val().trim(), indE, 2);
                }
            });
        ...
        function Cancel() {
            window.location = "../Master/DistRegionCompanyMapping.aspx";
        }
        ...
        function btnSubmit_Click() {
            ...
            $('#tblEmpClaimLevel  > tbody > tr').each(function (row, tr) {
                ...
                if (Company == "" && IsChange == "1") {
                    $.unblockUI();
                    IsValid = false;
                    ModelMsg("Please select Sales Organization : " + (parseInt(row) + 1), 3);
                    return false;
                }
            ...
            $('#tblEmpClaimLevel  > tbody > tr').each(function (row, tr) {
                ...
                if ((CompanyName != "" && Region != "" && LogoFile != "") && (IsChange == "1")) {
                    totalItemcnt = 1;
            ...
            if (IsValid) {
                var sv = $.ajax({
                    url: 'DistRegionCompanyMapping.aspx/SaveData',
                    type: 'POST',
                    //async: false,
                    dataType: 'json',
                    // traditional: true,
                    data: JSON.stringify({ hidJsonInputUnitMapping: UserUnitMappingData, OptionId: option }),
                    contentType: 'application/json; charset=utf-8'
                });
        ...
        function GetReport() {
            ...
            if ($('.chkIsReport').find('input').is(':checked')) {
                ...
                $('.gvEmpClaimLevelHistory tbody').empty();
                $.ajax({
                    url: 'DistRegionCompanyMapping.aspx/LoadReport',
                    type: 'POST',
                    dataType: 'json',
                    async: false,
                    contentType: 'application/json; charset=utf-8',
                        ...
                        <table id="tblEmpClaimLevel" class="table table-bordered nowrap" border="1" tabindex="8" style="border-collapse: collapse; font-size: 10px; width: 100%">
                            <thead>
                                <tr class="table-header-gradient">
                                    ...
                                    <th style="width: 20%; padding-left: 10px !important;">Logo</th>
                                    <th style="width: 20%; padding-left: 10px !important;">Logo Preview</th>
